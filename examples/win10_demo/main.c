/**
 * @file main.c
 * @brief Windows 10 demo – minimal ConvAI SDK usage example.
 *
 * Demonstrates the complete engine lifecycle:
 *   create → start → send messages/audio → poll events → stop → destroy
 *
 * Build via CMake:
 *   mkdir build && cd build && cmake .. -DCONVAI_PLATFORM=win10
 *   cmake --build .
 */
...
int main(void)
{
    ...
    printf("========================================\n");
    printf("  ConvAI SDK – Windows 10 Demo\n");
    ...
    const char *hello = "Hello from Windows!";
    ...
    printf("[DEMO] Exit (press any key to close)\n");
    return EXIT_SUCCESS;
}
